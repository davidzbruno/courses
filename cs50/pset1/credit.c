...
// checks to see if the entire string passed is is numeric
...
// finds the sums of the digits in the number n
int digitSum(int n)
{
    int sum = 0;
    while (n > 0)
...
/*
 * Based on the type of card that is beieng sent it this method cheecks to see what card number was passed in as a parameter
...
{    
    ...
    else if ((strlen(str) == 16) && (strncmp(str, "51", 2) == 0 || strncmp(str, "52", 2) == 0 || strncmp(str, "53", 2) == 0
                                     || strncmp(str, "54", 2) == 0 || strncmp(str, "55", 2) == 0))
    {
        return "MASTERCARD";
    }
...
int main(void)
{
    //allocate size to store the card number
    ...
    //returns the name of the card that was checked
    printf("%s\n", cardCheck(input));
    return 0;
}
